/**
 * Edit Network Dialog Component
 * Form dialog for editing existing blockchain networks
...
export function EditNetworkDialog({ open, onOpenChange, network }: EditNetworkDialogProps) {
  ...
  useEffect(() => {
    if (network && open) {
      setFormData({
        ...
        network_type: network.network_type || 'evm',
        chain_id: network.chain_id || '',
        withdrawal_fee: String(network.withdrawal_fee || 0),
        withdrawal_fee_percent: String(network.withdrawal_fee_percent || 0),
        min_withdrawal: String(network.min_withdrawal || 0),
        max_withdrawal: network.max_withdrawal !== null ? String(network.max_withdrawal) : '',
        withdrawal_enabled: network.withdrawal_enabled ?? true,
        deposit_enabled: network.deposit_enabled ?? true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.network_type) {
      toast.error('Name and network type are required');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(data.error || 'Failed to update network');
        return;
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update network configuration, fees, and withdrawal limits
          </DialogDescription>
        </DialogHeader>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="basic" className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="code">Network Code (Immutable)</Label>
                <Input
                  id="code"
                  value={formData.code}
                  disabled
                ...
                <p className="text-xs text-text-tertiary">
                  Network code cannot be changed after creation
                </p>
              </div>
