/**
 * Edit Base Token Dialog
 * Form for editing existing base token definitions
 */
...
interface EditBaseTokenDialogProps {
  token: BaseToken | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function EditBaseTokenDialog({ token, open, onOpenChange }: EditBaseTokenDialogProps) {
  ...
  useEffect(() => {
    if (token && open) {
      setFormData({
        ...
        symbol: token.symbol || '',
        name: token.name || '',
        token_type: token.token_type || 'native',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/admin/base-tokens/${token.id}`, {
        method: 'PUT',
      ...
      if (!response.ok) {
        toast.error(data.error || 'Failed to update base token');
        return;
      }

      toast.success('Base token updated successfully!');
      onOpenChange(false);
      router.refresh();
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Edit Base Token</DialogTitle>
          <DialogDescription>
            Update the token definition. Changes will affect all deployments using this token.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="code" value={formData.code} disabled className="bg-bg-tertiary" />
              <p className="text-xs text-text-tertiary">Code cannot be changed after creation</p>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-text-tertiary">Is this token active?</p>
              </div>
              <Switch
                id="is_active"
